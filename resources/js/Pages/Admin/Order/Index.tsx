...
import { Clock, CheckCircle2, ChefHat, XCircle, User, Hash, FileText } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { useEffect } from 'react'; // 💡 1. Tambahkan import useEffect dari React
...
export default function Index({ orders }: OrdersProps) {
    ...
    const ordersList: Order[] = orders?.data ?? (Array.isArray(orders) ? orders : []);
        ...
        if (typeof window !== 'undefined' && (window as any).Echo) {
            (window as any).Echo.channel('dapur-channel')
                .listen('OrderCreated', (e: any) => {
                    ...
                    // Bunyikan efek suara notifikasi bawaan browser (opsional, sangat berguna bagi koki)
                    try {
                        const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2869/2869-600.wav');
                        audio.volume = 0.5;
                        audio.play();
                    } catch (err) {
                        console.log("Audio play blocked by browser path policy");
                    }

                    // Tampilkan notifikasi toast hijau yang cantik
                    toast.success(`🔔 Pesanan Baru! Meja ${table} atas nama ${customer} masuk antrean.`, {
                        duration: 6000,
                        position: 'top-right',
                        style: {
                            background: '#1c2e24',
                    ...
                    // Segarkan list kartu pesanan dapur secara diam-diam di background (instant update)
    ...
    const updateStatus = (id: string, newStatus: string) => {
        router.patch(`/admin/orders/${id}/status`, {
            ...
            preserveScroll: true,
            onSuccess: () => toast.success(`Pesanan berhasil diperbarui ke: ${newStatus}`),
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-col gap-6">
                {/* Header */}
                <div>
                    <h1 className="text-2xl font-bold text-matcha-900 dark:text-cream-50">Daftar Pesanan Dapur</h1>
                    <p className="text-sm text-matcha-600 dark:text-cream-300">Pantau dan proses semua pesanan pelanggan secara real-time.</p>
                </div>
                ...
                {ordersList.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl bg-white py-16 text-center shadow-sm dark:bg-matcha-900 border border-matcha-100 dark:border-matcha-800">
                        <ChefHat size={48} className="text-matcha-300 animate-pulse mb-3" />
                        <p className="text-matcha-500 font-medium">Belum ada pesanan masuk saat ini.</p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center justify-between mb-4 mt-1">
                                        <div className="flex items-center gap-2 rounded-xl bg-matcha-50 px-3 py-1.5 dark:bg-matcha-950 border border-matcha-100 dark:border-matcha-800">
                                            <Hash size={14} className="text-matcha-500" />
                                            <span className="text-sm font-bold text-matcha-800 dark:text-cream-100">Meja {order.table_number}</span>
                                        </div>
                                        <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${getStatusStyle(order.status)}`}>
                                            {order.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 text-sm text-matcha-700 dark:text-cream-200 mb-3">
                                        <User size={16} className="text-matcha-400 shrink-0" />
                                        ...
                                        {order.items?.map((item) => (
                                            <div key={item.id} className="flex justify-between items-start text-sm">
                                                <span className="text-matcha-600 dark:text-cream-300 flex-1 pr-2">
                                                    {item.name} <strong className="text-matcha-800 dark:text-foam-400 ml-1">x{item.qty}</strong>
                                                </span>
                                                <span className="font-medium text-matcha-700 dark:text-cream-200">{formatRupiah(item.price * item.qty)}</span>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {order.notes && (
                                        <div className="mt-3 flex gap-2 rounded-xl bg-cream-50 p-2.5 text-xs text-matcha-700 border border-matcha-100 dark:bg-matcha-800/40 dark:text-cream-300 dark:border-matcha-800">
                                            <FileText size={14} className="text-matcha-400 shrink-0 mt-0.5" />
                                ...
                                <div>
                                    ...
                                    <div className="flex justify-between items-center mb-4">
                                        <span className="text-xs font-medium text-matcha-500">Total Pembayaran</span>
                                        <span className="text-base font-bold text-matcha-900 dark:text-foam-400">{formatRupiah(order.total)}</span>
                                    </div>
