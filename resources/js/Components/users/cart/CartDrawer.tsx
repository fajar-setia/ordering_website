...
export default function CartDrawer({ open, onClose }: CartDrawerProps) {
    ...
    const handleCheckoutSubmit = async (formData: LocalCheckoutData) => {
    if (cart.length === 0) {
        console.warn("⚠️ Checkout dibatalkan: Keranjang belanja kosong.");
        return;
    }
    ...
    try {
        ...
        if (newOrder && newOrder.id) {
            localStorage.setItem("active_order_id", String(newOrder.id));
            window.dispatchEvent(new Event("local-storage-update"));
        } else {
            console.error("❌ GAGAL: Response sukses, tapi properti 'order' atau 'id' tidak ditemukan.");
        }
        ...
        if (error.response) {
            console.error("Status Error Backend:", error.response.status);
            console.error("Detail Error Validasi/Database dari Laravel:", error.response.data);
        } else {
            console.error("Error Koneksi/Frontend:", error.message);
        }
        
        toast.dismiss('checkout-error'); // Hapus toast error sebelumnya jika ada
            ...
            (t) => (
                <div className="flex flex-col gap-2.5">
                    <p className="text-xs font-medium text-matcha-900 dark:text-cream-100">
                        ...
                        <strong className="font-semibold text-matcha-700 dark:text-foam-400">
                            {name}
                        </strong>{" "}
                        dari keranjang?
                    </p>
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between border-b border-matcha-100 px-5 py-4 dark:border-matcha-800">
                                <div className="flex items-center gap-2">
                                    <ShoppingBag
                                        size={19}
                                        className="text-matcha-600 dark:text-foam-400"
                                    />
                                        ...
                                        {cart.length === 0 ? (
                                            ...
                                            >
                                                <span className="flex h-16 w-16 items-center justify-center rounded-full bg-matcha-100 dark:bg-matcha-800">
                                                    <ShoppingBag
                                                        size={26}
                                                        className="text-matcha-400 dark:text-cream-500"
                                                    />
                                                </span>
